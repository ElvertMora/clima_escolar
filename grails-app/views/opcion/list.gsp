			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="label" title="${message(code: 'opcion.label.label', default: 'Label')}" />
					
						<g:sortableColumn property="textoOpcion" title="${message(code: 'opcion.textoOpcion.label', default: 'Texto Opcion')}" />
				...
				<g:each in="${opcionInstanceList}" status="i" var="opcionInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${opcionInstance.id}">${fieldValue(bean: opcionInstance, field: "label")}</g:link></td>
					
						<td>${fieldValue(bean: opcionInstance, field: "textoOpcion")}</td>
